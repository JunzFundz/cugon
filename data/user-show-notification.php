        ...
        while ($rows = $result->fetch_assoc()) { ?>
            ...
            <ol class="relative border-s border-gray-200 dark:border-gray-700 overflow-scroll">
                <li class="mb-10 ms-4">
                    ...
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Hi! <?= $rows['email'] ?></h3>
                    <p class=" text-base font-normal text-gray-500 dark:text-gray-400">Request is <?= $rows['status'] ?></p>
                    <?php
                    if (!empty($rows['message'])) {
                        echo "<p class='mb-4 text-base font-normal text-red-500 dark:text-gray-400'>Message: {$rows['message']}</p>";
                    } else {
                        echo "<p class='mb-4 text-base font-normal text-gray-500 dark:text-gray-400'>No message available</p>";
                    }
                    ?>
                    <a href="Transactions.php?userId=<?php echo $user_id; ?>" class="inline-flex items-center px-4 py-2 text-sm font-medium text-gray-900 bg-white border border-gray-200 rounded-lg hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:outline-none focus:ring-gray-100 focus:text-blue-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700 dark:focus:ring-gray-700">View <svg class="w-3 h-3 ms-2 rtl:rotate-180" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 10">
